...
#' @export
#'
#' @examples graph.sample(x=bss of one sample, final_metadata_for_saving, trophic_affiliation_of_organisms.csv, bv.type="elli", living.only=TRUE)

graph.sample <- function(x, final_metadata_for_saving, taxo, bv.type="elli", living.only=T) {
...
p7<-ggplot(plot_data) +
 ...
 #st_crop(xmin=lonmin, xmax=lonmax, ymax=latmax, ymin=latmin)
  meta.x <- filter(final_metadata_for_saving, sample_id==unique(x$sample_id))
