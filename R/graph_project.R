...
#' @param final_metadata_for_saving metadata table generated by "check_metadata" used to do the analysis
...
#' @export
#'
#' @examples graph.project(x=bss table of all the project, final_metadata_for_saving, taxo=trophic_affiliation_of_organisms.csv, bv.type="elli", living.only=TRUE)
graph.project <- function(x, final_metadata_for_saving, taxo, bv.type="elli", living.only=T) {

  x <- filter(x, type==bv.type) %>% merge(taxo, "object_annotation_hierarchy", all.x=T)
  t <- final_metadata_for_saving %>% select(sample_id, sample_num, object_time, object_date) %>%
