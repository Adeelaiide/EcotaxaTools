...
#' @param selected_metadata_for_bss metadata table generated by "check_metadata" or similar 
#'
...
compute_bv <- function(path, output, selected_metadata_for_bss=NULL) {
  ...
  if(!is.null(selected_metadata_for_bss)) {
    for(i in unique(data$unique_id)) {
      meta <- selected_metadata_for_bss[selected_metadata_for_bss$unique_id==i,]
